...
async def cevir(event):
    ...
    if islem in ("foto", "photo"):
        ...
        if efekt in EFEKTLER:
            ...
            await event.client.send_file(event.chat_id, "output.mp3", reply_to=rep_msg, caption="@TheFastUserBot `ile efekt uygulandı✅.`")
        ...
        try:
            await event.client.send_file(event.chat_id, "out.mp3",reply_to=rep_msg, caption='@TheFastUserBot ilə səsə çevrildi.✅')
        except:
            os.remove(video)
