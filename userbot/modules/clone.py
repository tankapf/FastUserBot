...
async def klon(event):
    ...
    await event.edit("Hesab Klonlanır 🚬")

    try:
        user = await bot(GetFullUserRequest(inp))
    except ValueError:
        await event.edit("Klonlamaq üçün tağ vəya birisinin mesajına yanıt ver🤨")
    ...
    await event.edit("**Səni @TheFastSup Tərəfindən Klonladım**")
...
async def revert(event):
    if not (USERINFO or os.path.exists(PHOTO)):
        await event.edit("Onsuzda Klonlanma Olmayıb 🤦🏻‍♂️")
        return
    if USERINFO:
