...
async def logo_gen(event):
    ...
    if os.path.exists(fayladi):
        await event.client.send_file(
            event.chat_id,
            file=fayladi,
            caption=f"[ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ 🔋](https://t.me/TheFastUserBot) `sizin üçün logo hazırlad`",
            force_document=True,
        )
        os.remove(fayladi)
        await event.delete()
...
Help = CmdHelp('logo')
Help.add_command('logo', '<yazı>', '`FAST userbot sizin istəyinizə uyğun logo hazırlayar`. `Modulu yoxlamaq üçün` @FastSohbet gəlin')
Help.add_info('@KenandiBrat_010 `tərəfindən hazırlandı`')
Help.add()
