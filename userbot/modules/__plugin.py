    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modul uğurla yükləndi!**\n__Modulun komandaları və işlədilişi haqqında məlumat üçün__ `.fast {cmdhelp}` __yazın.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modul uğurla yükləndi!**\n__Modulun komandaları və işlədilişi haqqında məlumat üçün__ `.fast {dosyaAdi}` __yazın.__')
