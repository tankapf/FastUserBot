...
UPSTREAM_REPO_URL = os.environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/FastUserBot/FastUserBot.git")
...
if not BLACKLIST_CHAT: #Əgər ayarlanıbsa Fast support qrupu əlavə olunur.
        ...
        async def inline_handler(event):
            ...
            if event.query.user_id == uid and query == "@FastSupp":
                ...
                result = await builder.article(
                    ...
                    text=f"**ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ İşləyir 🔋** [ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ](https://t.me/fastsupp) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sayfa:** 1/{veriler[0]}",
                    buttons=veriler[1],
                    link_preview=False
                )
...
Hesabınızı bot'a çevirə bilərsiz və bunları işlədə bilərsiz. Unutmayın, siz başqasının botunu idarə etməssiniz! Altdakı GitHub adresində bütün qurulum detayları var""",
        ...
        async def sayfa(event):
            if not event.query.user_id == uid: 
                return await event.answer("Hey! Mənim mesajlarımı düzəltməyə çalışma! Özünə bir ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ 🔋 qur.", cache_time=0, alert=True)
            sayfa = int(event.data_match.group(1).decode("UTF-8"))
            veriler = butonlastir(sayfa, CMD_HELP)
            await event.edit(
                f"**ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ İşləyir 🔋** [ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ](https://t.me/fastuserbot) __İşləyir...__\n\n**Yüklənən Modul Sayı:** `{len(CMD_HELP)}`\n**Sayfa:** {sayfa + 1}/{veriler[0]}",
                buttons=veriler[1],
                link_preview=False
            )
        ...
        async def bilgi(event):
            if not event.query.user_id == uid: 
                return await event.answer("Hey! Mənim mesajlarımı düzəltməyə çalışma! Özünə bir ꜰᴀꜱᴛ ᴜꜱᴇʀʙᴏᴛ İşləyir 🔋 qur..", cache_time=0, alert=True)
